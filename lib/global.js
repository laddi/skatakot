...
var params = {};
if (Meteor.isServer && Meteor.settings.filestore && Meteor.settings.filestore.path) {
	params.path = Meteor.settings.filestore.path;
}

Images = new FS.Collection('Images', {
	stores: [
		new FS.Store.GridFS('thumbs', _.extend(params, { transformWrite: createThumb })),
		new FS.Store.GridFS('images', params)
	],
	filter: {
...
Schemas.Huts = new SimpleSchema({
	...
	status: {
		type: String,
		allowedValues: [ 'status-in-use', 'status-needs-repair', 'status-destroyed', 'status-rebuilt' ],
		autoform: {
			options: [
